                    ...
                    <div class="row">
                        ...
                        <div class="col-12 col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="date" disabled class="form-control" id="tgl" value="<?= date('Y-m-d') ?>" name="tgl">
                            </div>
                        </div>
                        ...
                        <div class="col-12 col-sm-12" style="overflow: auto;">
                            ...
                            <table id="tambahbelanja" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                    <tr id="tr_{{ $key }}" @if ($v['up']) style="background-color: #a9a9a966" @endif>
                                        <td @if (!$v['up']) style="padding-left: 50px;" @endif>
                                            @if (!$v['up'])
                                            <a class="btn btn-danger btn-sm" onclick="hapusbelanja({{ $v['id'] }}, {{ $key }})" style="margin-top: 3px;position: absolute;z-index: 9;left:20px;"><i class="fa fa-times"></i>
                                            </a>
                                            @endif

                                            @if ($v['bahan_id'])
                                            <select @if ($v['up']) disabled @endif name="nama[]" onchange="clicknama(this.value, {{ $key }}, {{ $v['bahan_id'] }})" id="nama_{{ $key }}" class="form-control select2 select2-danger" data-dropdown-css-class="select2-danger" style="width: 100%;margin-left:100px;">
                                                ...
                                                <option value="{{ $bhn['id'] }}" @if ($v['bahan_id']==$bhn['id']) selected @endif>
                                                    {{ $bhn['nama'] }}
                                                </option>
                                                @endforeach
                                            </select>
                                            @else
                                            <input title="Nama Barang" type="text" class="form-control" onchange="$('#FormBelanja').submit()" id="nama_{{ $key }}" @if ($v['up']) disabled @endif value="{{ $v['nama'] }}" placeholder="Nama Barang" name="nama[]">
                                            @endif
                                            @if (!$v['up'])
                                        ...
                                        <td>
                                            ...
                                            @if ($v['bahan_id'])
                                            <select disabled id="kategori_{{ $key }}" class="form-control select2 select2-danger" data-dropdown-css-class="select2-danger" style="width: 100%;">
                                                <option selected="true" disabled="disabled">Pilih</option>
                                                <option value="Item" @if ($v['bahan_id']> 0) selected @endif>Item
                                                </option>
                                                <option value="Oprasional" @if ($v['kategori']=='Oprasional' ) selected @endif>
                                                    Oprasional
                                                </option>
                                                <option value="Supplay" @if ($v['kategori']=='Supplay' ) selected @endif>
                                                    Supplay
                                                </option>
                                                <option value="ART" @if ($v['kategori']=='ART' ) selected @endif>ART
                                                </option>
                                                <option value="DAPRO" @if ($v['kategori']=='DAPRO' ) selected @endif>DAPRO
                                                </option>
                                            </select>
                                            <input @if ($v['up']) disabled @endif type="hidden" name="kategori[]" id="kategori_val_{{ $key }}" value="<?= $v['kategori'] ?>">
                                            @else
                                            <select @if ($v['up']) disabled @endif onchange="$('#FormBelanja').submit()" id="kategori_{{ $key }}" name="kategori[]" class="form-control select2 select2-danger" data-dropdown-css-class="select2-danger" style="width: 100%;">
                                                <option selected="true" disabled="disabled">Pilih</option>
                                                <option value="Oprasional" @if ($v['kategori']=='Oprasional' ) selected @endif>
                                                    Oprasional
                                                </option>
                                                ...
                                                <option value="DAPRO" @if ($v['kategori']=='DAPRO' ) selected @endif>
                                                    DAPRO
                                                </option>
                                            </select>
                                            @endif
                                        ...
                                        <td>
                                            <div class="row">
                                                <div class="col">
                                                    <input @if ($v['up']) disabled @endif type="number" class="form-control" id="qty_{{ $key }}" value="{{ $v['qty'] }}" onchange="hitung_belanja(this.value, {{ $key }})" placeholder="Qty" name="qty[]">
                                                </div>
                                                <div class="col">
                                                    <select @if ($v['up']) disabled @endif name="uombelanja[]" onchange="$('#FormBelanja').submit()" id="uombelanja_{{ $key }}" class="form-control select2 select2-danger" data-dropdown-css-class="select2-danger" style="width: 100%;">
                                                        <option selected="true" disabled="disabled">UOM
                                                        </option>
                                                        4
                                                        @foreach ($satuan as $stn)
                                                        <option value="{{ $stn['singkat'] }}" @if ($v['uom']==$stn['singkat']) selected @endif>
                                                            {{ $stn['nama'] }}
                                                        </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="col">
                                                    <input @if ($v['up']) disabled @endif title="Harga 1 nya" data-toggle="tooltip" data-placement="top" type="number" class="form-control" onchange="hitung_belanja(this.value, {{ $key }})" id="harga_{{ $key }}" value="{{ $v['harga'] }}" placeholder="Harga" name="harga[]">
                                                </div>
                                            </div>
                                        </td>
                                        <td id="item_{{ $key }}">
                                            <div style="display: flex;">
                                                ...
                                                <div class="input-group">
                                                    <input @if ($v['up']) disabled @endif onchange="hitung_belanja(this.value,{{ $key }})" style="text-align: right;max-width:100px;" type="number" value="{{ $v['stock'] }}" name="stock[]" id="stock_{{ $key }}" class="form-control" placeholder="Stock">
                                                    <div class="input-group-append"><span class="input-group-text" id="belanja_uom_{{ $key }}">
                                                            {{ $v['stock_uom'] }}</span></div>
                                                </div>

                                                <input @if ($v['up']) disabled @endif type="hidden" name="stock_uom[]" value="{{ $v['stock_uom'] }}" id="stock_satuan_{{ $key }}">

                                                &nbsp;

                                                <div class="input-group">
                                                    <div class="input-group-prepend"><span class="input-group-text" id="belanja_uom_{{ $key }}">Rp.</span>
                                                    </div>

                                                    <input @if ($v['up']) disabled @endif title="Harga 1 nya" data-toggle="tooltip" data-placement="top" onchange="hitung_belanja(this.value,{{ $key }})" style="text-align: right;max-width:100px;" type="number" value="{{ $v['stock_harga'] }}" name="stock_harga[]" id="stock_harga_{{ $key }}" class="form-control" placeholder="Stock Harga">

                                                </div>
                                                @else
                                                <input type="hidden" @if ($v['up']) disabled @endif value="{{ $v['stock'] }}" name="stock[]" class="form-control">
                                                <input type="hidden" @if ($v['up']) disabled @endif name="stock_uom[]" value="{{ $v['stock_uom'] }}" id="stock_satuan_{{ $key }}">
                                                <input type="hidden" @if ($v['up']) disabled @endif value="{{ $v['stock_harga'] }}" name="stock_harga[]">
                                        ...
                                        <td> <input @if ($v['up']) disabled @endif type="text" class="form-control" value="{{ $v['ket'] }}" id="ket" placeholder="Ket" name="ket[]" onchange="$('#FormBelanja').submit()">
                                        </td>
                                        <td>
                                            <select @if ($v['up']) disabled @endif style="border: unset;background: transparent;" name="hutang[]" onchange="$('#FormBelanja').submit()">
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="7">
                                            <a class="btn btn-success btn-block" id="add_row_belanja"><i class="fas fa-plus"></i></a>
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 col-sm-6">
                            <div style="padding: 30px; font-size: xx-large; ">
                                <b>Total :</b>
                                <h2 style="font-size:50px;border-bottom: 2px #007bff solid"><b id="total_seluruh">{{ 'Rp ' . number_format($totaldapro + $totalart + $totalsup + $totalopr + $totalitem, 0, ',', '.') }}</b>
                                </h2>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function upbelanja(id) {
        Swal.fire({
            title: 'Yakin ingin upload ?',
            text: "Tindakan ini akan menambah inventory",
            icon: 'warning',
