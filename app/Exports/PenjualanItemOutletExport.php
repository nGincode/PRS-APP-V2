    ...
    FromArray
{
    ...
    public function headings(): array
    {
        ...
        $result[] = [''];
        $result[] = ['INFO'];
        $store = POSBillItem::select('tujuan')->distinct()->where('store_id', $this->store)->whereBetween('tgl', [$this->tgl_awal, date('Y-m-d', strtotime("+1 day", strtotime($this->tgl_akhir)))])->with('Store', 'PosBill')->get();
        $totalperoutletall = 0;
        foreach ($store as $va) {
            $vall = POSBillItem::where('tujuan', $va['tujuan'])->where('store_id', $this->store)->whereBetween('tgl', [$this->tgl_awal, date('Y-m-d', strtotime("+1 day", strtotime($this->tgl_akhir)))])->orderBy('tgl', 'ASC')->get();
            $totalperoutlet = 0;
            foreach ($vall as $vv) {
    ...
    public function array(): array
    {
        ...
        $data = POSBillItem::select('tujuan', 'nama')->distinct()->where('store_id', $this->store)->whereBetween('tgl', [$this->tgl_awal, date('Y-m-d', strtotime("+1 day", strtotime($this->tgl_akhir)))])->get();

        $result = [];
        $no = 1;
        $totalall = 0;
        foreach ($data as $key => $value) {
            $val = POSBillItem::where('tujuan', $value['tujuan'])->where('nama', $value['nama'])->where('store_id', $this->store)->whereBetween('tgl', [$this->tgl_awal, date('Y-m-d', strtotime("+1 day", strtotime($this->tgl_akhir)))])->orderBy('tgl', 'ASC')->get();
