    ...
    FromArray
{
    ...
    public function array(): array
    {
        ...
        $data = [];
        $pos = POSBillItem::where('store_id', $this->store)->whereBetween('tgl', [$this->tgl_awal, date('Y-m-d', strtotime("+1 day", strtotime($this->tgl_akhir)))])->with('Store', 'Posbill')->orderBy('tgl', 'ASC')->orderBy('tgl', 'ASC')->get();
        $order = Orderitem::where('up', true)->where('logistik', $this->store)->whereBetween('tgl', [$this->tgl_awal, date('Y-m-d', strtotime("+1 day", strtotime($this->tgl_akhir)))])->with('Store')->get();
