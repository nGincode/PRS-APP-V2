    ...
    FromArray
{
    ...
    public function array(): array
    {
        ...
        $data = POSBillItem::select('nama')->distinct()->where('store_id', $this->store)->whereBetween('tgl', [$this->tgl_awal, $this->tgl_akhir])->orderBy('tgl', 'ASC')->orderBy('tgl', 'ASC')->get();

        $result = [];
        $no = 1;
        $totalall = 0;
        foreach ($data as $key => $value) {
            $val = POSBillItem::where('nama', $value['nama'])->where('store_id', $this->store)->whereBetween('tgl', [$this->tgl_awal, $this->tgl_akhir])->orderBy('tgl', 'ASC')->orderBy('tgl', 'ASC')->get();
